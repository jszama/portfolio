      ...
      <div className="absolute top-12 right-2 bg-[var(--secondary-dark-background)] border-2 border-[var(--primary-magenta)] text-[var(--primary-text)] rounded-md shadow-lg px-4 py-2">
        <ul className="space-y-2">
          {NavigationItems.map((item) => (
            <li key={item.name} className="hover:text-[var(--hover-magenta)] transition-all duration-200">
              <a href={item.href} onClick={(e) => { e.preventDefault(); document.querySelector(item.href)?.scrollIntoView({ behavior: 'smooth' }); }}>
                {item.name}
              </a>
            </li>
          ))}
