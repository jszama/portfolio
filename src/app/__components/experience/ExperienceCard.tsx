...
import { Experience } from './assets/experiences'
import { Button } from '@headlessui/react';
import Modal from '../Modal';

export default function ExperienceCard({ company, company_icon, position, duration, description, techStack, readMore, modalContent, isExpanded, onToggle }: Experience & { isExpanded: boolean, onToggle: () => void }) {
    ...
    return (
        <>
        <article id={company} className="flex flex-col snap-start items-center gap-y-1 sm:gap-y-2">
            <div
                className={
                    `bg-[var(--muted-background)] rounded-lg hover:scale-[1.01] hover:drop-shadow-[0_0_4px_var(--hover-magenta)] flex items-start font-bold  mb-2 sm:max-h-80 max-h-64 h-80 relative overflow-hidden transition-all duration-500 ease-out will-change-transform ${isExpanded ? 'w-[90vw] sm:w-[36rem]' : 'w-[11rem] sm:w-[14rem]'}`
                }
                style={{
                ...
                >
                {/* Unexpanded View */}
                <div className="flex flex-col py-3 px-4 shrink-0 cursor-pointer" onClick={onToggle}>
        ...
        { modalContent && (
            <Modal isOpen={isOpen} setIsOpen={setIsOpen} className="bg-[var(--primary-dark-background)] w-full h-full" >
                {modalContent}
            </Modal >
        )}
        </>
